...
as $$
  select case
    ...
    when bucket_id = 'testimonial-avatars' then
      object_name ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/avatar/[a-z0-9](?:[a-z0-9._-]*[a-z0-9])?\.(?:avif|jpe?g|png|webp)$'
    else
      false
  end;
$$;
...
comment on function public.is_content_image_bucket(text) is
  'Returns true when a storage bucket is part of the public content image strategy, including testimonial avatars.';

comment on function public.is_valid_content_image_path(text, text) is
  'Validates storage object paths for case study images, blog images, and testimonial avatars.';
